...
def main_menu_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("📚 الخطط الدراسية", callback_data="study_plans_menu")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الخطط الدراسية
        ...
        await query.edit_message_text("📚 اختر الكلية أو المسار لعرض الخطط الدراسية:", reply_markup=study_plans_menu_keyboard())
    elif data == "plan_eng":
        await query.edit_message_text("⚙️ خطط المسار الهندسي:", reply_markup=plan_eng_keyboard())
    elif data == "plan_health":
        await query.edit_message_text("🩺 خطط المسار الصحي:", reply_markup=plan_health_keyboard())
    elif data == "plan_biz":
        await query.edit_message_text("💼 خطط إدارة الأعمال:", reply_markup=plan_biz_keyboard())
    elif data == "plan_applied":
        await query.edit_message_text("📊 خطط الكلية التطبيقية:", reply_markup=plan_applied_keyboard())
    elif data == "plan_langs":
        await query.edit_message_text("🗣️ خطط كلية اللغات والترجمة:", reply_markup=plan_langs_keyboard())
    elif data == "plan_arts":
        await query.edit_message_text("🧪 خطط كلية العلوم والآداب:", reply_markup=plan_arts_keyboard())
    elif data == "plan_cs":
        await query.edit_message_text("💻 خطط علوم الحاسب ونظم المعلومات:", reply_markup=plan_cs_keyboard())
    elif data == "plan_paths":
        await query.edit_message_text("🛤️ خطط المسارات التحضيرية:", reply_markup=plan_paths_keyboard())
