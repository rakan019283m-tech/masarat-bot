...
# ----------------- قوائم شروط الاستمرار الفرعية (بدون أرقام في الأزرار) -----------------

def gpa_conditions_keyboard():
    keyboard = [
        [InlineKeyboardButton("🩺 الطب والجراحة", callback_data="cond_medicine"),
         InlineKeyboardButton("🦷 طب الأسنان", callback_data="cond_dentistry")],
        [InlineKeyboardButton("💊 دكتور صيدلي", callback_data="cond_pharmacy"),
         InlineKeyboardButton("💉 التمريض", callback_data="cond_nursing")],
        [InlineKeyboardButton("🧪 العلوم الطبية التطبيقية", callback_data="cond_applied_medical")],
        [InlineKeyboardButton("💻 علوم الحاسب ونظم المعلومات", callback_data="cond_cs")],
        [InlineKeyboardButton("⚙️ كلية الهندسة", callback_data="cond_engineering")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تفاصيل شروط التخصصات الفردية (معدل واضح داخل محتوى الرسالة وتنسيق هادئ)
    elif data == "cond_medicine":
        text = (
            "🩺 **تخصص الطب والجراحة**\n"
            "──────────────────\n"
            "⭐ **المعدل المطلوب:** `4.25` فأعلى للاستمرار في التخصص.\n\n"
            "📌 **شروط إضافية وأحكام:**\n"
            "• يجوز النزول عن المعدل المذكور في حال توفر مقاعد شاغرة.\n"
            "• في حال عدم تحقيق المعدل، يتاح التحويل لتخصص آخر ضمن نفس المسار (بشرط توفر مقاعد ومعدل مناسب).\n"
            "• عند اكتمال مقاعد المسار، يتم التحويل لأحد برامج البكالوريوس التي لا تتطلب سنة مشتركة أو إلى الدبلوم."
        )
        ...
        text = (
            "🦷 **تخصص طب الأسنان**\n"
            "──────────────────\n"
            "⭐ **المعدل المطلوب:** `3.75` فأعلى للاستمرار في التخصص.\n\n"
            "📌 **شروط إضافية وأحكام:**\n"
            "• يجوز النزول عن المعدل المذكور في حال توفر مقاعد شاغرة.\n"
            "• يتاح التحويل لبرامج المسار ذاته عند عدم تحقيق الحد الأدنى.\n"
            "• التحويل لبرامج البكالوريوس الأخرى أو الدبلوم عند عدم توفر مقاعد في المسار."
        )
        keyboard = [[InlineKeyboardButton("⬅️ رجوع لقائمة الشروط", callback_data="gpa_conditions")],
                    [InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="main_menu")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        ...
        text = (
            "💊 **تخصص دكتور صيدلي**\n"
            "──────────────────\n"
            "⭐ **المعدل المطلوب:** `3.25` فأعلى للاستمرار في التخصص.\n\n"
            "📌 **شروط إضافية وأحكام:**\n"
            "• يجوز النزول عن المعدل المذكور في حال توفر مقاعد شاغرة.\n"
            "• التحويل متاح لبرامج المسار ذاته عند عدم استيفاء المعدل المطلوب.\n"
            "• عند امتلاء مقاعد المسار، يتم التحويل لبكالوريوس لا يتطلب سنة مشتركة أو الدبلوم."
        )
        keyboard = [[InlineKeyboardButton("⬅️ رجوع لقائمة الشروط", callback_data="gpa_conditions")],
                    [InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="main_menu")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        ...
        text = (
            ...
            "📌 **شروط إضافية وأحكام:**\n"
            "• يجوز النزول عن المعدل المذكور في حال توفر مقاعد شاغرة.\n"
            "• إمكانية التحويل لبرامج المسار عند عدم استيفاء المعدل وشريطة توفر المقاعد.\n"
            "• التحويل للبرامج التي لا تتطلب سنة مشتركة أو الدبلوم عند عدم توفر مقاعد."
        )
        keyboard = [[InlineKeyboardButton("⬅️ رجوع لقائمة الشروط", callback_data="gpa_conditions")],
                    [InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="main_menu")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        ...
        text = (
            "🧪 **تخصصات العلوم الطبية التطبيقية**\n"
            "──────────────────\n"
            "⭐ **المعدل المطلوب:** `3.00` فأعلى للاستمرار في التخصصات.\n\n"
            "📌 **شروط إضافية وأحكام:**\n"
            "• يجوز النزول عن المعدل المذكور في حال توفر مقاعد شاغرة.\n"
            "• التحويل لبرامج المسار ذاته عند عدم تحقيق شرط المعدل وتوفر المقاعد الشاغرة.\n"
            "• التحويل لبرامج البكالوريوس بدون سنة مشتركة أو الدبلوم عند امتلاء المقاعد."
        )
        keyboard = [[InlineKeyboardButton("⬅️ رجوع لقائمة الشروط", callback_data="gpa_conditions")],
                    [InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="main_menu")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        ...
        text = (
            ...
            "──────────────────\n"
            "⭐ **المعدل المطلوب:** `3.25` فأعلى للاستمرار في التخصصات.\n\n"
            "📌 **شروط إضافية وأحكام:**\n"
            "• يجوز النزول عن المعدل المذكور في حال توفر مقاعد شاغرة.\n"
            "• التحويل لبرامج المسار عند عدم تحقيق الحد الأدنى للمعدل.\n"
            "• التحويل لبرامج البكالوريوس الأخرى غير المطلوبة للسنة المشتركة أو الدبلوم."
        )
        keyboard = [[InlineKeyboardButton("⬅️ رجوع لقائمة الشروط", callback_data="gpa_conditions")],
                    [InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="main_menu")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        ...
        text = (
            ...
            "📌 **شروط إضافية وأحكام:**\n"
            "• يجوز النزول عن المعدل المذكور في حال توفر مقاعد شاغرة.\n"
            "• التحويل لبرامج المسار ذاته شريطة استيفاء المعدل والمقاعد الشاغرة.\n"
            "• التحويل لأحد برامج البكالوريوس التي لا تتطلب السنة المشتركة أو الدبلوم عند عدم توفر مقاعد."
        )
        keyboard = [[InlineKeyboardButton("⬅️ رجوع لقائمة الشروط", callback_data="gpa_conditions")],
                    [InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="main_menu")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
